...
io.on('connection', function(socket) {
  ...
  && socket.handshake.headers.referer.indexOf(config.host + config.dashboardEndpoint) > -1) {
    ...
    console.log("+ L'utilisateur " + socket.id + " vient de se connecter sur la page dashboard.html");
  ...
  socket.on('disconnect', function() {
    ...
    console.log("- L'utilisateur " + socket.id + " vient de ce déconnecter");
...
function computeStats(){
  ...
  console.log("computeStats() = " + computeStatsObj);
return computeStatsObj;
}
...
function computePageCounts() {
  ...
  console.log("computePageCounts() = " + pageCounts);
  return pageCounts;
}
...
function computeRefererCounts() {
  ...
  console.log("computeRefererCounts() = " + referrerCounts);
  return referrerCounts;
}

// get the total active users on our site
function getActiveUsers() {
  console.log("getActiveUsers() = " + Object.keys(visitorsData).length);
